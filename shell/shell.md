...
```
#!/bin/bash
readonly curl='www.baicu.com'
curl='yahoo.com'
echo $curl
```

运行结果 可见当发生报错时候还是会向下执行

```shell
[miaoyi@localhost test]$ bash readonly.sh
readonly.sh: line 4: curl: readonly variable
www.baicu.com 
```
...
\b	后退
\c	抑制（不显示）输出结果中任何结尾的换行字符（只在%b 格式指示符控制下的参数字符串中有效），而且，任何留在参数里的字符、任何接下来的参数以及任何留在格式字符串中的字符，都被忽略 不换行
\f	换页（formfeed）
\n	换行
...
- 完整的表达式要被单引号 **` `** 包含，注意这个字符不是常用的单引号，在 Esc 键下边。
